...
const Register = () => {
  ...
  const schema = yup
    .object({
      email: yup.string().email('Field expects an email address').required('Email is required'), //text inside email is shown if email is invalid
      password: yup.string()
      ...
      password2: yup.string().required('Password confirmation is required').oneOf([yup.ref('password'), null], 'Passwords must match')
